...
class Pendaftaran extends CI_Controller
{
   ...
   public function index()
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        // mencetak string 
        $pdf->Cell(45, 7, '', 0, 0, 'C');
        $pdf->Cell(290, 7, 'PUSKESMAS ', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(30, 7, '', 0, 0, 'C');
        $pdf->Cell(330, 7, 'INDONESIA', 0, 1, 'C');
        $pdf->Cell(70, 7, '', 0, 0, 'C');
        $pdf->Cell(250, 7, 'Telepon : (0264) 1111111. E-mail: puskesmas@gmail.com', 0, 1, 'C');
...
function cetakbulan(){
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        // mencetak string 
        $pdf->Cell(45, 7, '', 0, 0, 'C');
        $pdf->Cell(230, 7, 'PUSKESMAS', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(30, 7, '', 0, 0, 'C');
        $pdf->Cell(270, 7, 'INDONESIA', 0, 1, 'C');
        $pdf->Cell(70, 7, '', 0, 0, 'C');
        $pdf->Cell(190, 7, 'Telepon : (0264) 1111111. E-mail: puskesmas@gmail.com', 0, 1, 'C');
  ...
  function cetakantrian($id){
        ...
        $pdf->Cell(90,5,'PUSKESMAS ',0,0,'C');
        $pdf->Cell(10,5,'',0,0,'C');
        $pdf->setFont('Arial','B',10);
        $pdf->Cell(90,5,'',0,1,'C');
        $pdf->setFont('Arial','B',8);
        $pdf->Cell(90,5,'INDONESIA',0,0,'C');
        $pdf->Cell(10,5,'',0,0,'C');
        $pdf->setFont('Arial','',7);
        $pdf->Cell(90,5,'',0,1,'L');
        ...
        $pdf->Cell(10,4,'',0,1,'C');
        $pdf->Cell(22,4,'Dokter',0,0,'L');
        $pdf->Cell(36,4,': '.$sql_pasien->nama_dokter,0,0,'L');
        $pdf->Cell(10,4,'',0,1,'C');
        $pdf->Cell(22,4,'Poli',0,0,'L');
        $pdf->Cell(36,4,': '.$sql_pasien->nama_poli,0,0,'L');
        $pdf->Cell(10,4,'',0,1,'C');
        $pdf->Ln(2);
   ...
   function rujukan_action(){
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(235, 7, 'PUSKESMAS ', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(235, 7, '          INDONESIA', 0, 1, 'C');
        $pdf->Cell(235, 7, 'Telepon : (0264) 1111111. E-mail: puskesmas@gmail.com', 0, 1, 'C');
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        //$pdf->Image('', )
        // mencetak string 
        $pdf->Cell(190, 7, 'PUSKESMAS ', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(190, 7, '          INDONESIA.', 0, 1, 'C');
        $pdf->Cell(190, 7, 'Telepon : (0264) 1111111. E-mail: puskesmas@gmail.com', 0, 1, 'C');
